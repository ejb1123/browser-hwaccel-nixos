...
if [ "${#CAPABLE_NODES[@]}" -eq 0 ]; then
  ...
  note "opens the wrong node. Pointing both at the other device fixes that:"
  note ""
  note "  hardware.browserHwaccel.vendor     = \"intel\";  # or amd / nvidia"
  note "  hardware.browserHwaccel.pciAddress = \"${CAPABLE_NODES[0]}\";"
  note ""
  warn "BUT read docs/hybrid-gpu.md before you do."
  note "Pinning video to a GPU that is not driving your displays means every"
  note "decoded NV12 frame is imported across GPUs. On at least one measured"
  note "system that fails outright -- gbm_bo_import errors, three GPU-process"
  note "crashes, and then Chromium disables accelerated video encode AND"
  note "decode for the rest of the session. There is no flag that keeps"
  note "encode while dropping decode."
  note ""
  note "Prefer the GPU that drives your displays, even with worse codec"
  note "support. Pin a different one only if it is the only GPU with a codec"
  note "you need -- and then test with real video playback before relying"
  note "on it. Some hybrid pairings are fine; you cannot tell without trying."
fi
